...
uint8_t dataT[PLD_S]; 
uint32_t counter = 0;  // Счетчик сообщений
...
int main(void)
{
  ...
  printf("NRF24 Transmitter Started\r\n");
 ...
 csn_high();
  ...
  while (1)
  {
    ...
    // Включаем светодиод перед передачей
    ...
    char message[PLD_S];
    snprintf(message, sizeof(message), "Hello NRF! #%lu", counter++);
    
    // Копируем сообщение в буфер для передачи
    strncpy((char*)dataT, message, PLD_S - 1);
    dataT[PLD_S - 1] = '\0'; // Гарантируем null-terminator
    
    // Читаем статус перед передачей
    uint8_t status_before = nrf24_r_reg(STATUS, 1);
    ...
    // Читаем статус после передачи
    uint8_t status_after = nrf24_r_reg(STATUS, 1);
    
    // Держим светодиод включенным 100мс для видимости
    HAL_Delay(100);
    
    // Выключаем светодиод после передачи
    HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, GPIO_PIN_RESET);
    
    // Выводим расширенный статус передачи через UART
    printf("Sent: %s | Status: %d | Before: 0x%02X | After: 0x%02X\r\n", 
           message, val, status_before, status_after);
    
    // Задержка 900мс (итого с предыдущими 100мс = 1 секунда)
    HAL_Delay(900);
